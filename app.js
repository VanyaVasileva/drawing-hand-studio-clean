...
const HANDS={
  ...
  cream:'assets/hand-cream-sleeve.png',sage:'assets/hand-sage-sleeve.png',pink:'assets/hand-pink-sleeve.png',dark:'assets/hand-dark-sleeve.png',brownOatmeal:'assets/hand-brown-oatmeal.png'
};
const HAND_META={
  default:{tipX:14.8,tipY:34.4,scale:1},cream:{tipX:9.4,tipY:26.2,scale:1.319},sage:{tipX:11.0,tipY:28.1,scale:1.292},pink:{tipX:13.8,tipY:27.4,scale:1.308},dark:{tipX:8.5,tipY:28.1,scale:1.280},brownOatmeal:{tipX:10.8,tipY:24.5,scale:1.398}
};
...
function tracker(vw,vh){
  const tw=Math.min(600,vw),th=Math.max(2,Math.round(vh*tw/vw)),cell=6,cols=Math.ceil(tw/cell),rows=Math.ceil(th/cell),N=cols*rows;
  track.width=tw;track.height=th;
  let prev=null,raw=null,smooth=null,vel={x:0,y:0},idle=999,missLocal=0;
  const sxScale=vw/tw,syScale=vh/th;
  ...
  const cellPoint=(k,count,weight,sumX,sumY)=>({x:sumX[k]/weight[k],y:sumY[k]/weight[k],count:count[k],mean:weight[k]/count[k]});
  const toSource=p=>({x:p.x*sxScale,y:p.y*syScale});
  function calm(target,isGlobal){
    if(!target){if(idle>45)smooth=null;return smooth}
    if(!smooth){smooth={...target};return smooth}
    let dx=target.x-smooth.x,dy=target.y-smooth.y,d=Math.hypot(dx,dy);
    if(d<0.8)return smooth;
    if(isGlobal||d>95){
      const step=Math.min(d,Math.max(9,Math.min(24,8+d*.09)));
      smooth={x:smooth.x+dx*step/d,y:smooth.y+dy*step/d};
    }else{
      const a=d<5?.22:d<14?.32:d<35?.48:.62;
      smooth={x:smooth.x+dx*a,y:smooth.y+dy*a};
    }
    return smooth;
  }
  return()=>{
    const r=read();if(!prev){prev=r;return{pos:smooth,idle}}
    const sens=+c.sensitivity.value,thr=Math.max(16,sens*.92);
    ...
    prev=r;
    const frac=changed/(tw*th),cands=[];
    for(let k=0;k<N;k++)if(count[k]>=2&&weight[k]>0){const p=cellPoint(k,count,weight,sumX,sumY);p.base=Math.log1p(p.count)*1.7+p.mean*.018;cands.push(p)}
    let target=null,isGlobal=false;
    if(raw&&idle<=45){
      const rx=raw.x/sxScale,ry=raw.y/syScale,vx=vel.x/sxScale,vy=vel.y/syScale,sp=Math.hypot(vx,vy),px=rx+vx*.65,py=ry+vy*.65;
      const radius=Math.min(110,Math.max(34,42+sp*1.8));
      let best=null;
      for(const p of cands){
        const dx=p.x-rx,dy=p.y-ry,dist=Math.hypot(dx,dy);if(dist>radius)continue;
        const pd=Math.hypot(p.x-px,p.y-py);let score=p.base-pd*.11-dist*.018;
        if(sp>1.2){const proj=(dx*vx+dy*vy)/sp,perp=Math.abs(dx*vy-dy*vx)/sp;score+=Math.max(-3,Math.min(6,proj*.08))-perp*.045}
        if(!best||score>best.score)best={...p,score};
      }
      if(best){target=toSource(best);missLocal=0}else missLocal++;
    }else missLocal++;
    if(!target&&frac<.075&&(missLocal>=2||!raw)){
      let best=null;
      for(const p of cands){
        if(p.count<3||p.count>180)continue;
        let score=p.base;
        if(raw){const rx=raw.x/sxScale,ry=raw.y/syScale,dist=Math.hypot(p.x-rx,p.y-ry);score-=Math.min(240,dist)*.004}
        if(p.y<42)score+=1.5;
        if(!best||score>best.score)best={...p,score};
      }
      if(best){target=toSource(best);isGlobal=!!raw;missLocal=0}
    }
    if(target){
      if(raw){const dx=target.x-raw.x,dy=target.y-raw.y;vel={x:vel.x*.58+dx*.42,y:vel.y*.58+dy*.42}}else vel={x:0,y:0};
      raw=target;idle=0;
    }else{
      idle++;vel={x:vel.x*.7,y:vel.y*.7};if(idle>45){raw=null;missLocal=0}
    }
    return{pos:calm(raw,isGlobal),idle}
  }
}
